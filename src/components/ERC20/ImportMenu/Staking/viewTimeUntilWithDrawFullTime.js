...
const WithdrawFulltime = ({ web3Staking, refreshDataGrid, tokenData }) => {
  ...
  const onClickApprove = async () => {
    ...
    try {
      ...
      var re = await web3Staking.methods
        .viewTimeUntilWithDrawFullTime(accounts[0])
        .call();
      successMessage = `${parseInt(re / 60 + 1) + " minutes to go"}`;
      ...
      <Grid item xs={12}>
        ...
        >
          {data.loading ? (
            ...
            "view Time Until WithDraw FullTime"
          )}
        </Button>
      </Grid>
      <Grid item xs={12}>
