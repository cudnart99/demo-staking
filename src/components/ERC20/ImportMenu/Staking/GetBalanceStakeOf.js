...
const GetBalanceStakeOf = ({ web3Staking, refreshDataGrid, tokenData }) => {
  ...
  const onClickApprove = async () => {
    ...
    let successMessage = "";
    try {
      var re = await web3Staking.methods.getBalanceStakeOf(data.arg1).call();
      successMessage = `${re / Math.pow(10, 18) + " tokens"}`;
  ...
  return (
    <Grid container>
      <Grid item xs={12}>
        ...
        >
          {data.loading ? <CircularProgress size={25} /> : "Get Balance Stake Of"}
        </Button>
      </Grid>
      <Grid item xs={12}>
        <TextField
...
export default GetBalanceStakeOf;
