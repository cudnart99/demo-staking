...
const SetRewardsDuration = ({ web3Staking, refreshDataGrid, tokenData }) => {
  ...
  const onClickApprove = async () => {
    ...
    try {
      ...
      await web3Staking.methods
        .setRewardsDuration(data.arg2)
        .send({ from: accounts[0] });
      successMessage = `Set duration staking equal to ${data.arg2} seconds`;
  ...
  return (
    <Grid container>
      <Grid item xs={12}>
        ...
        >
          {data.loading ? <CircularProgress size={25} /> : "set duration stake"}
        </Button>
      </Grid>
      <Grid item xs={12}>
        {/* <TextField
...
export default SetRewardsDuration;
