...
const NotifyRewardAmount = ({ web3Staking, refreshDataGrid, tokenData }) => {
  ...
  const onClickApprove = async () => {
    ...
    try {
      ...
      await web3Staking.methods
        .notifyRewardAmount(amountToApprove)
        .send({ from: accounts[0] });
      successMessage = `Set ${data.arg2} token to be rewards , start staking`;
  ...
  return (
    <Grid container>
      <Grid item xs={12}>
        ...
        >
          {data.loading ? <CircularProgress size={25} /> : "Update staking contract"}
        </Button>
      </Grid>
      <Grid item xs={12}>
        {/* <TextField
...
export default NotifyRewardAmount;
