...
const GetBalanceStake = ({ web3Staking, refreshDataGrid, tokenData }) => {
  ...
  const onClickBalanceOf = async () => {
    let rawBalance = "";
    try {
      rawBalance = await web3Staking.methods.balanceOf(data.arg1).call();
    } catch (error) {
  ...
  return (
    <Grid container>
      <Grid item xs={12}>
        ...
        >
          Balance Staking of (address owner)
        </Button>
        <Typography
          variant="subtitle1"
          noWrap
...
export default GetBalanceStake;
