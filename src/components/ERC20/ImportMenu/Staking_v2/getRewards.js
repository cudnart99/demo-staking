...
const GetReward = ({ web3Staking, refreshDataGrid, tokenData }) => {
  ...
  const onClickApprove = async () => {
    ...
    try {
      ...
      await web3Staking.methods.getReward().send({ from: accounts[0] });
      successMessage = `Get reward token successful. `;
  ...
  return (
    <Grid container>
      <Grid item xs={12}>
        ...
        >
          {data.loading ? <CircularProgress size={25} /> : "Get Reward"}
        </Button>
      </Grid>
      <Grid item xs={12}>
        {/* <TextField
